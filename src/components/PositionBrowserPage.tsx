    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">深度解读全国 100+ 企事业单位、大厂、科研院所热门岗位说明书</p>
        </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5 mb-8 shadow-2xs space-y-5">
        ...
        <div>
          ...
          {subIndustries.length > 0 && (
            <div className="flex items-center gap-3 pl-16 py-1.5 bg-slate-50/50 rounded-xl mt-1.5 border border-slate-100/50">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-1">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <select
              ...
              className="bg-slate-50 border border-slate-200 rounded-xl p-2.5 text-xs font-semibold text-slate-700 focus:bg-white focus:outline-none transition-all cursor-pointer"
            >
        ...
        {hasActiveFilters && (
          <div className="pt-2 border-t border-dashed border-slate-100 flex justify-between items-center bg-slate-50/20 -mx-5 -mb-5 p-4 rounded-b-2xl">
            <span className="text-xs text-slate-500 font-semibold flex items-center gap-1">
              <Filter className="w-3.5 h-3.5 text-blue-500" />
              已启用筛选条件，为您匹配到 <strong className="text-blue-600 font-extrabold">{filteredPositions.length}</strong> 个岗位
            </span>
            <button
              onClick={handleClearFilters}
              className="text-xs font-bold text-red-500 hover:text-red-600 transition-colors cursor-pointer flex items-center gap-1 bg-red-50 hover:bg-red-100/60 px-3 py-1.5 rounded-xl border border-red-100"
            >
      ...
      {paginatedPositions.length > 0 ? (
            ...
            {paginatedPositions.map((pos) => (
              ...
              >
                <div>
                  ...
                  <h3 className="text-base font-extrabold text-slate-900 group-hover:text-blue-600 transition-colors leading-tight line-clamp-1">
                    {pos.title}
                  </h3>
          ...
          {filteredPositions.length > visibleCount && (
            <div className="flex justify-center pt-4">
              <button
                ...
                className="px-8 py-3 bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white text-xs font-bold rounded-xl transition-all shadow-md hover:shadow-lg hover:-translate-y-0.5 cursor-pointer flex items-center gap-1.5"
              >
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-16 text-center shadow-2xs">
          ...
          <button
            onClick={handleClearFilters}
            className="px-6 py-2 bg-blue-50 text-blue-600 border border-blue-100 hover:bg-blue-100 text-xs font-bold rounded-xl transition-all cursor-pointer"
          >
            重置所有筛选条件
          </button>
        </div>
