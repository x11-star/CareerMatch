...
export default function ShareModal({ onClose }: ShareModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-sm rounded-3xl border border-career-line bg-career-surface p-6 shadow-2xl sm:p-8">
        ...
        <div className="mb-6 grid grid-cols-2 gap-3 text-xs font-semibold">
          ...
          <button onClick={handleCopyLink} className="flex cursor-pointer flex-col items-center gap-2 rounded-2xl border border-career-line bg-career-bg p-3 text-career-ink transition-colors hover:border-career-primary hover:bg-career-primary-soft">
...
function UnavailableShareButton({ icon, label }: { icon: React.ReactNode; label: string }) {
  return <button disabled className="flex cursor-not-allowed flex-col items-center gap-2 rounded-2xl border border-career-line bg-career-surface-muted p-3 text-career-muted opacity-80">{icon}{label}</button>;
}
